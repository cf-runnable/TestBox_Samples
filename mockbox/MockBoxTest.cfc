<cfcomponent extends="testbox.system.testing.BaseSpec" displayname="MockBox Suite">
	...
	<cfscript>
		...
		function testStubWithInheritance(){
			mock = getMockBox().createStub( extends="testbox.system.EventHandler" );
			$assert.isTrue( isInstanceOf( mock, "testbox.system.EventHandler" ) );
		}
		
		function testStubWithImplements(){
			mock = getMockBox().createStub( implements="testbox.system.cache.ICacheProvider" );
			$assert.isTrue( isInstanceOf( mock, "testbox.system.cache.ICacheProvider" ) );
		}
