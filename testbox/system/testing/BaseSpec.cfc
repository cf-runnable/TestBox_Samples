...
component{
	...
	variables.$mockBox = this.$mockBox 	= new testbox.system.testing.MockBox();
	// Assertions object
	variables.$assert = this.$assert = new testbox.system.testing.Assertion();
	// Custom Matchers
	this.$customMatchers 		= {};
	// Utility object
	this.$utility 				= new testbox.system.core.util.Util();
	...
	*/
	remote function runRemote( string testSpecs="", string testSuites="", boolean debug=false, string reporter="simple" ) output=true{
		var runner = new testbox.system.testing.TestBox( bundles="#getMetadata(this).name#", reporter=arguments.reporter );
