...
component accessors="true"{
	...
	* @reporter.hint The type of reporter to use for the results, by default is uses our 'simple' report. You can pass in a core reporter string type or an instance of a testbox.system.testing.reports.IReporter
	...
	* @bundles.hint The path, list of paths or array of paths of the spec bundle CFCs to run and test
	* @directory.hint The directory to test which can be a simple mapping path or a struct with the following options: [ mapping = the path to the directory using dot notation (myapp.testing.specs), recurse = boolean, filter = closure that receives the path of the CFC found, it must return true to process or false to continue process ]
	* @reporter.hint The type of reporter to use for the results, by default is uses our 'simple' report. You can pass in a core reporter string type or an instance of a testbox.system.testing.reports.IReporter. You can also pass a struct if the reporter requires options: {type="", options={}}
	* @labels.hint The list or array of labels that a suite or spec must have in order to execute.
	* @options.hint A structure of configuration options that are optionally used to configure a runner.
	...
	* @testSpecs.hint A list or array of test names that are the ones that will be executed ONLY!
	*/
	testbox.system.testing.TestResult function runRaw( 
		any bundles,
		any directory,
		any labels,
		struct options,
		...
		var results = new testbox.system.testing.TestResult( bundleCount=arrayLen( variables.bundles ), 
	...
	*/
	private any function buildReporter( required reporter ){
		...
		switch( arguments.reporter ){
			case "json" : { return new "testbox.system.testing.reports.JSONReporter"(); }
			case "xml" : { return new "testbox.system.testing.reports.XMLReporter"(); }
			case "raw" : { return new "testbox.system.testing.reports.RawReporter"(); }
			case "simple" : { return new "testbox.system.testing.reports.SimpleReporter"(); }
			case "dot" : { return new "testbox.system.testing.reports.DotReporter"(); }
			case "text" : { return new "testbox.system.testing.reports.TextReporter"(); }
			case "junit" : { return new "testbox.system.testing.reports.JUnitReporter"(); }
			case "antjunit" : { return new "testbox.system.testing.reports.ANTJUnitReporter"(); }
			case "console" : { return new "testbox.system.testing.reports.ConsoleReporter"(); }
			case "min" : { return new "testbox.system.testing.reports.MinReporter"(); }
			case "tap" : { return new "testbox.system.testing.reports.TapReporter"(); }
			case "doc" : { return new "testbox.system.testing.reports.DocReporter"(); }
			case "codexwiki" : { return new "testbox.system.testing.reports.CodexWikiReporter"(); }
			default: {
	...
	*/
	private function testBundle(
		...
		if( structKeyExists( target, "run" ) ){
			// Run via BDD Style
			new testbox.system.testing.runners.BDDRunner( options=variables.options )
				.run( target, arguments.testResults );
		}
		else{
			// Run via xUnit Style
			new testbox.system.testing.runners.UnitRunner( options=variables.options )
				.run( target, arguments.testResults );
		}
	...
	*/ 
	private any function getBundle( required bundlePath ){
		...
		var familyPath 	= "testbox.system.testing.BaseSpec";
		...
		// Else virtualize it
		var baseObject 			= new testbox.system.testing.BaseSpec();
